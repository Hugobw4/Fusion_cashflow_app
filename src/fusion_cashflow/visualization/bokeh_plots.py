...
def plot_sensitivity_bokeh(outputs, config, sensitivity_df):
    ...
    p = figure(
        width=width,
        height=height,
        x_range=Range1d(-0.5, 6.5),
        y_range=Range1d(-0.1 * height, 1.1 * height),
    ...
    patch_source = ColumnDataSource(
        data=dict(
            ...
            color=patch_colors,
            label=patch_labels,
            percent=patch_percents,
            value=patch_values,
            source=patch_sources,
            target=patch_targets,
        )
    )
    p.patches(
        "xs", "ys", color="color", alpha=0.7, line_color=None, source=patch_source
    )

    # --- 5. Draw nodes as rectangles (pill style) ---
    node_source = ColumnDataSource(
        data=dict(
            x=[n["x"] for n in nodes],
            y=[n["y"] for n in nodes],
            w=[n["w"] for n in nodes],
            h=[n["h"] for n in nodes],
            color=[n["color"] for n in nodes],
            label=[n["label"] for n in nodes],
        )
    )
    # TODO: Use pill/rounded nodes when Bokeh supports it (rounded_rect or border_radius)
    p.rect(
        "x",
        "y",
        "w",
    ...
    # --- 7. Hover tool for ribbons ---
    hover = HoverTool(
        tooltips=[
            ("Flow", "@source → @target"),
            ("Value", "@label"),
            ("% of Revenue", "@percent{0.0%}"),
        ],
        renderers="auto",
    )
    p.add_tools(hover)
    ...
    # TODO: Animate ribbon highlight on hover for extra polish.
